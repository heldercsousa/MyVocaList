...
- **No C#-side string normalization for search/deduplication — HARD RULE:** Never call `ToLowerInvariant()`, `ToUpperInvariant()`, `RemoveDiacritics()`, or any equivalent C#-side normalization in services, repositories, or entity constructors for the purpose of search, duplicate detection, or uniqueness. Never add `*Normalized` shadow columns (properties or DB columns whose sole purpose is storing a lowercased/accent-stripped variant of another column). `ToLowerInvariant()` handles case only — "café" ≠ "cafe" — making it silently incorrect for accented names. The correct pattern: declare `.UseCollation(CollationConstants.Default)` on searchable/unique columns in EF entity configurations; use `EF.Functions.Collate(column, CollationConstants.Default)` in LINQ queries; let the DB handle normalization at query time via the `NOCASE_NOACCENT` collation registered by `CollationInterceptor`. Each Infra layer defines its own collation constant (`CollationConstants.cs`); there is no shared cross-Infra collation abstraction.
