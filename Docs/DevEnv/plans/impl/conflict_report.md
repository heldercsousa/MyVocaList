...
|----|-----|-------------|----------|-----------|--------|
| B-01 | 🔴 | workflow.md | Approved | Add qualifier to Wave handoff: inject verbatim signatures only when new wave has explicit dependency on prior wave's output AND artifact is not already in a file the subagent will naturally read. Rule A (paths only) unchanged. | Done |
| B-02 | 🔴 | workflow.md | Approved | Calibration table (Rule B) is canonical. Update "When to skip SDD" — small isolated change minimum artifact changes from "Descriptive commit message" to "tasks.md entry + commit message". Only true exemptions (typos, comment fixes, pure docs) keep commit-message-only. | Done |
| B-03 | 🔴 | workflow.md | Rejected | Both spec rot frequency prescriptions removed. /project:review after every task + Rule 3a session-end ritual are the spec-drift detection mechanism. Spec rot indicators list may be kept as reference within Rule 3a but prescribed frequency rules are deleted. | Done |
| B-04 | 🔴 | review.md + testing.md + workflow.md | Approved | Standardize AC traceability matrix to: AC ID | Criterion (short) | Implementation location | Test method — across review.md §9, testing.md "Traceability matrix," and workflow.md Rule 5. | Done |
...
| B-06 | 🔴 | testing.md + review.md | Approved | review.md §9: add Level C exemption — document no-test decision in task-log. testing.md Level C: add "if a Level C task has ACs, document the no-test decision in the task-log — it will be scrutinized at review." Both files converge: Level C + AC = allowed but must be explicitly task-log documented. | Done |
| B-07 | 🔴 | workflow.md + testing.md | Approved | /project:review stays in CLAUDE.md as human command reference only. Stop hook is the authoritative Verifier dispatch trigger — document this in workflow.md. Remove /project:review from Implementor exit checklist scope (already absent from implementor.md). Create orchestrator.md in .claude/agents/ consolidating post-wave verification, Verifier dispatch, and wave management responsibilities currently scattered in workflow.md. | Done |
| B-08 | 🔴 | CLAUDE.md | Approved | CLAUDE.md § Constitutional Constraints becomes routing only: constraint names + one-line rationale + pointer to authoritative file. No inline detail. Specialized files (code-principles.md, constraints-registry.md, skills) are authoritative. Merge Unamendable list into Constitutional Constraints as [Unamendable] markers — remove separate subsection. | Done |
| B-09 | 🔴 | CLAUDE.md + code-principles.md | Approved | code-principles.md § Architecture Constraints is the authoritative source for layer dependency rules. CLAUDE.md § Architecture becomes a 2-line pointer only. "Business logic in Services" consolidates to code-principles.md — all other 3 locations become one-line pointers. CLAUDE.md duplication removed entirely. | Done |
| W-01 | 🟡 | workflow.md | Approved | Rule 7 is canonical for session-start spec reading. Replace content in "Mandatory spec reads at session start" and "Context reset discipline" with cross-references to Rule 7. | Done |
| W-02 | 🟡 | workflow.md | Approved | Add "with no diagnostic improvement" qualifier to Build retry cap definition. Rename 3-attempt dispatch escalation to "3-dispatch escalation protocol" to distinguish from build retries. | Done |
| W-03 | 🟡 | workflow.md | Approved | Add authorization qualifier to Living spec protocol: write back only decisions within the task authorized scope, per Spec ownership constraint. Align Rule 2a to explicitly permit the one-line note pattern as a named exception. | Done |
| W-04 | 🟡 | workflow.md | Approved | Add footnote to Rule 2 table: dotnet ef migrations add generates the scaffold via CLI (main agent shell); the subagent then edits the migration file. | Done |
| W-05 | 🟡 | workflow.md | Approved | >= 2 layers always means full three-file spec. Update SDD decision table row "Change touches >= 2 layers" to "All three files" not design.md only. One authoritative answer. | Done |
| W-06 | 🟡 | workflow.md | Approved | Restate Verifier subagent section: Optional except for Architectural review-lane tasks, where it is mandatory (see Rule 2b). | Done |
| W-07 | 🟡 | workflow.md | Approved | Renumber rules sequentially and reorder: 1 -> 2 (absorb 2a/2b as subsections) -> 3 (absorb 3a) -> 4 -> 5 -> 6 -> 7 -> 8. A reader must be able to navigate by number. | Done |
| W-08 | 🟡 | workflow.md | Approved | Move ACTIVE-CONSIDERATIONS.md, Multi-session handoff, Context exhaustion, Context reset discipline, Wave completion discovery briefs from Rule 2 to Rule 7. Rule 2 stays focused on subagent mechanics. | Done |
| W-09 | 🟡 | workflow.md | Approved | Move "Mandatory spec reads at session start" to Rule 7 and merge with Session Start reading order. Remove orphaned section from Rule 2. | Done |
| W-10 | 🟡 | workflow.md | Approved | Merge Spec size calibration, Spec ceremony calibration, and "When to skip SDD" into one consolidated decision table near top of Rule 1. Columns: task type, effort, spec required, ceremony level, required artifacts. | Done |
| W-11 | 🟡 | workflow.md + CLAUDE.md | Approved | Remove "Regeneration test practice" from workflow.md. "Rebuild test" is canonical; add "and the test suite" to its context sources. Make CLAUDE.md entry a one-line pointer to workflow.md rebuild test section. | Done |
| W-12 | 🟡 | workflow.md | Approved | Move Rule 2a and 2b to immediately follow Rule 2 (or absorb as named subsections). W-07 renumbering pass applies this automatically. | Done |
| W-13 | 🟡 | review.md | Approved | Keep one canonical "spec updated when code deviates" check in section 6 at Blocker. Replace redundant checks in sections 7, 8, 10, 12 with cross-reference to section 6 Spec Consistency. | Done |
| W-14 | 🟡 | review.md | Approved | Group sections 6-12 under "Spec and AC Verification" parent heading. Designate section 6 as primary check. Move section 9 to immediately follow AC coverage checks in section 6. | Done |
| W-15 | 🟡 | workflow.md | Approved | Add to exit checklist step 1: "A task with zero Blocker findings per /project:review may be set To Review. Warning findings must be documented in the task-log." | Done |
| W-16 | 🟡 | review.md | Approved | Consolidate "Interface signatures match design.md" check to section 6 at Blocker severity. Remove from sections 8 and 12. | Done |
| W-17 | 🟡 | review.md | Approved | Add to review.md: "Test quality criteria per testing.md must also pass (see Test Quality Audit Checklist) before To Review." | Done |
| W-18 | 🟡 | review.md + workflow.md | Approved | Add cross-reference in each direction: review.md "After Review" -> workflow.md Rule 3a; workflow.md Rule 3a -> review.md "After Review." | Done |
| W-19 | 🟡 | review.md | Approved | Section 9 is canonical for AC-to-test mapping. In sections 6 and 8, replace AC coverage checks with "see section 9 AC Traceability." | Done |
| W-20 | 🟡 | testing.md | Approved | In Tester/Builder Role Separation Rules item 1, add: "(Note: in a single-agent session, apply one-at-a-time discipline per One test at a time — Exception.)" | Done |
| W-21 | 🟡 | testing.md | Approved | Reorder sections so TDD Level Guidance precedes Mutation Testing / Stryker. Fixes forward-reference and is prerequisite for S-21. | Done |
| W-22 | 🟡 | code-principles.md + workflow.md | Approved | Move "Spec Language — Determinism" section to workflow.md Rule 1, adjacent to Acceptance criteria format. In code-principles.md, replace with one-line reference to workflow.md. | Done |
| W-23 | 🟡 | CLAUDE.md | Approved | Add to Continuous Enhancement: "Note: changes to CLAUDE.md or .claude/rules/*.md must follow the Amending These Rules process — including amend: commit prefix and changelog entry." | Done |
| W-24 | 🟡 | CLAUDE.md + workflow.md | Approved | Add "and the test suite" to workflow.md Rebuild test context sources. Make CLAUDE.md Spec Quality Check a pointer only to workflow.md rebuild test section. | Done |
| W-25 | 🟡 | code-principles.md + constraints-registry.md | Approved | constraints-registry.md is authoritative for runtime constraints. In code-principles.md, replace EF Core/SQLite section with pointer to constraints-registry.md. | Done |
| W-26 | 🟡 | code-principles.md | Approved | Resolved by W-22 action — moving Spec Language Determinism out of code-principles.md eliminates the misfit rule. No additional action needed. | Done |
| XF-01 | 🟡 | workflow.md | Approved | Add to pre-dispatch validation checklist: "For UI/coding tasks: confirm subagent briefing includes instruction to invoke myvocalist-coding skill per CLAUDE.md Skill and MCP Lookup." | Done |
| XF-02 | 🟡 | workflow.md | Approved | Add to Pre-task context gate checklist: "Required MCPs for this task are available (per CLAUDE.md MCP Availability Gate). If unavailable: set status blocked: MCP unavailable, stop." | Done |
| S-01 | 🟢 | workflow.md | Approved | Spec gap escalation is canonical. In Pre-task context gate, replace definition with "per Spec Gap Escalation protocol." | Done |
| S-02 | 🟢 | workflow.md | Approved | findings.md session artifact section is canonical. Remove inline content descriptions from Spike pattern and Discovery mode; add "see findings.md section" reference. | Done |
| S-03 | 🟢 | workflow.md | Approved | Spec ownership constraint is canonical. Add cross-references from Subagent scope constraint and Rule 2a to it. | Done |
| S-04 | 🟢 | workflow.md | Approved | Post-wave verification is canonical. Remove or merge "When to take back control" into it. Rule 2b cross-references. | Done |
| S-05 | 🟢 | workflow.md | Approved | Add explicit cross-references from Task atomization checklist and Task entry format to the Rule 1 Demo statement requirement definition. | Done |
| S-06 | 🟢 | workflow.md | Approved | Remove "Session resume rule" sentence from handoff protocol; add "See Rule 7 — Session Start Protocol." | Done |
| S-07 | 🟢 | workflow.md | Approved | In Subagent return protocol, replace commit+push steps with "Follow Subagent exit checklist steps 7-8." | Done |
| S-08 | 🟢 | workflow.md + review.md | Approved | Standardize to [YYYY-MM-DD] date placeholder in all instances across workflow.md and review.md. | Done |
| S-09 | 🟢 | workflow.md | Approved | Resolved as part of W-03 action — Living spec protocol gets the authorization qualifier. No separate action needed. | Done |
| S-10 | 🟢 | workflow.md | Approved | Add hook health verification as step 0 in Rule 7 session start reading order, or add cross-reference from Rule 7 to Hook health verification section at top of file. | Done |
| S-11 | 🟢 | workflow.md | Approved | Move Subagent exit checklist to be the last subsection of Rule 2, immediately before the Rule 3 separator. | Done |
| S-12 | 🟢 | workflow.md | Approved | Move "When to skip SDD" to immediately after the SDD Invariant at the start of Rule 1. Handled by W-10 consolidated table. | Done |
| S-13 | 🟢 | workflow.md | Rejected | Duplicate of W-12. W-07 renumbering pass handles Rule 2a/2b placement. No separate action. | Done |
| S-14 | 🟢 | review.md | Approved | Update review.md to use [YYYY-MM-DD] date placeholder. Companion to S-08. | Done |
| S-15 | 🟢 | review.md | Approved | Promote "After Review" to a numbered section within the review checklist (e.g., section 13 Enhancement Check). Text says not optional — structure must match. | Done |
| S-16 | 🟢 | testing.md + review.md | Approved | Align test quality audit trigger to unconditional (run after every task). Remove the "if test files were changed" conditional qualifier from testing.md. | Done |
| S-17 | 🟢 | testing.md | Approved | In the anti-patterns table, add cross-reference to the Builder Must Not Modify Tests section for the escalation path. | Done |
| S-18 | 🟢 | testing.md + CLAUDE.md | Approved | Replace step-based preamble in testing.md with: "TDD applies to all new and modified Services, ViewModels, and Repositories." Update CLAUDE.md Rules Files testing.md description to remove "prerequisites for Step 3" phrase. | Done |
| S-19 | 🟢 | testing.md | Approved | Remove Prerequisites Before Step 3 section entirely — all prerequisites are resolved at this stage of the project. Archive context is in git history. | Done |
| S-20 | 🟢 | testing.md | Approved | Remove duplicate --- separator between Tester/Builder Role Separation and TDD Workflow sections. | Done |
| S-21 | 🟢 | testing.md | Approved | Move TDD Level Guidance to before Mutation Testing / Stryker section. Prerequisite resolved by W-21 reordering. | Done |
| S-22 | 🟢 | testing.md | Approved | Companion to W-20. Add note in Tester/Builder Role Separation referencing the one-at-a-time exception in One test at a time section. | Done |
| S-23 | 🟢 | CLAUDE.md | Approved | Resolved by B-08 action — once Unamendable list is merged into Constitutional Constraints, duplicate entries disappear. No separate action needed. | Done |
| S-24 | 🟢 | CLAUDE.md + code-principles.md | Approved | Remove Language section from code-principles.md. Add at top: "Language rule: English only — see CLAUDE.md Constitutional Constraints." | Done |
| S-25 | 🟢 | CLAUDE.md + code-principles.md | Approved | Add inline section pointers to CLAUDE.md Constitutional Role: naming conventions, DI registration, error handling each pointing to their code-principles.md section. | Done |
| S-26 | 🟢 | CLAUDE.md + code-principles.md | Approved | Move "Prefer composition over inheritance" from CLAUDE.md Architecture to code-principles.md C# Style. Remove from CLAUDE.md. | Done |
| S-27 | 🟢 | CLAUDE.md | Approved | Update CLAUDE.md Rules Files testing.md description: "testing.md — read before writing any test. Covers test types, naming, TDD workflow, and test project setup." Companion to S-18. | Done |
| S-28 | 🟢 | code-principles.md + constraints-registry.md | Approved | Remove ObservableRangeCollection section from constraints-registry.md. Replace with cross-reference to code-principles.md UI Thread Performance section. | Done |
| S-29 | 🟢 | code-principles.md | Approved | Add after each Global Usings list: "Verify against the projects GlobalUsings.cs — this list is a snapshot, not the authoritative source." | Done |
| S-30 | 🟢 | code-principles.md + constraints-registry.md | Approved | Add Incremental XAML edits entry to constraints-registry.md: Edit one XAML file -> build -> fix before editing the next. Rationale: XAML parser errors cascade. | Done |
| S-31 | 🟢 | code-principles.md | Approved | Add a brief table of contents or navigation header at the top of code-principles.md listing the main sections. | Done |
| S-32 | 🟢 | constraints-registry.md | Approved | Companion to S-28. Replace ObservableRangeCollection entry in constraints-registry.md with cross-reference to code-principles.md. | Done |
| S-33 | 🟢 | constraints-registry.md | Approved | Add Incremental XAML edits entry as described in S-30. | Done |
| XF-03 | 🟢 | workflow.md | Approved | Add one-sentence clarification distinguishing DevEnv/ (environment/session state files) from superpowers/plans/ (plan-execution artifacts) in workflow.md session artifacts section. | Done |
