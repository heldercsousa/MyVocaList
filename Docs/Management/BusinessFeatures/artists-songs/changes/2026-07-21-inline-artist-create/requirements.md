...
> **Spec updated [2026-07-21]:** supersedes the original BUG-008 clear-on-blur behavior; the prior test AC-B8-01 is repurposed to assert retain-on-no-match under REQ-ACREATE-03.
...
- **REQ-ACREATE-15 (BUG-060, Major)** `[added 2026-07-23 — T10 re-run #2]` — A locked artist field must be **changeable**. Tapping the clear (**X**) icon on a locked field **unlocks** it (`IsArtistLocked = false`) and clears the selection (`SelectedArtistId`/`SelectedArtistName`/`ArtistSearchText`), returning the field to a normal searchable state so the user can pick or create a different artist. Blur on a field the user **deliberately cleared** must **not** silently restore the previously-selected artist (the restore-prior-selection branch applies only when the user re-typed over an existing lock without clearing). Regression test at the VM seam: after clear, assert `IsArtistLocked == false` and selection cleared; assert a subsequent blur does not repopulate the prior artist.

- **REQ-ACREATE-16 (BUG-067, Critical)** `[added 2026-08-02 — T10 re-run #4]` — Changing the artist of an **already-saved** song must be persisted. After the user unlocks the Artist field (REQ-ACREATE-15), selects or inline-creates a different artist, and saves, the stored song's `ArtistId` is the newly selected artist's id; reopening the song shows the new artist. The edit path must send the currently selected artist to the Service layer, and the Service must write the artist FK on update — not only on create. The song-title uniqueness check on update is evaluated against the **effective (new) artist**, and an unknown artist id is rejected with a message, no write. Regression test at the Service seam (`UpdateSongAsync` persists a changed artist id) and at the ViewModel seam (edit-mode Save forwards `SelectedArtistId`). Mandatory Red→Green.
