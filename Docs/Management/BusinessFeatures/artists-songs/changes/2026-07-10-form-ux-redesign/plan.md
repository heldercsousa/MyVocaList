...
**tasks.md:** identical title · Risk High · Review lane Elevated · TDD Level A
**⚠ GAP-1 applies to Step 6 only** (see § Open spec gaps) — resolve with Helder at plan review or set `blocked: spec gap` at that step.
...
4. No match → transparent create (REQ-FORMUX-20 — **GAP-1**: route per Helder's A/B decision; Option A = `_songResolution.CommitAsync(candidate, ResolutionChoice.CreateNew, null, null)` with the ArtistCandidate carrying typed name + marked-for-create identity, then attach `_pendingRawUrls` via `ISongKaraokeUrlService`).
...
- [ ] **Step 6 (⚠ GAP-1 — needs Helder A/B decision first):** `Save_NoMatch_TransparentCreateWithMarkedForCreateIdentity` — `// [AC] REQ-FORMUX-20: artist + song in one atomic save; external identity persisted` (assert the chosen lever is invoked with an ArtistCandidate carrying the typed name + pending external identity; a failure surfaces via the existing tuple/snackbar path, never an escaping exception). Red → Green.
...
- **Open items:** GAP-1 (Task 12 Step 6) — Helder decision A/B required at plan review.
