...
| 2026-07-15 | **String trimming on persistence — centralized normalization analysis** | 🗺️ Plan | Goal: strings persisted to the DB should be trimmed (extension of BUG-046's query-side trimming) via one centralized Services-layer helper (search) + EF Core `ValueConverter`s (persistence). Gate: D1 (internal collapse: YES) + D2 (folder routing: approved) recorded 2026-07-15; D3 (persistence mechanism: EF Core `ValueConverter` in `EntityTypeConfiguration`, not per-Service-method calls — whitespace-in-storage reclassified as a data-integrity invariant, not business logic) recorded 2026-07-19; plan.md/tasks.md re-synced to D3 — awaiting Helder plan approval. Pointer: `DevCycleCraft/persisted-string-trimming/`. |
