...
| 2026-07-10 | **AppBar / SearchAppBar Interaction Redesign — page-nav pattern + persistent search bar** | 📋 Spec | Goal: kill the bar-swap search toggle — persistent MD3 standalone search bar hosted in `CrudListView`, SearchAppBar retired (hypothesis validated against MD3 + NN/g, Helder approved 2026-07-19). Gate: spec + four-gate governance (governed components). Pointer: `DevCycleCraft/appbar-searchbar-redesign/2026-07-19-persistent-searchbar-decision.md`. |
...
| 2026-07-19 | ↳↳ **Replace `AutocompleteMobileField` consumers with DX `AutoCompleteEdit`** | 🟡 In Progress | Goal: mature built-in autocomplete on all form consumers; unblocks BUG-027 → Artists & Songs Catalog. Gate: spec + plan reviewer-passed 2026-07-19 (Helder pre-approved autonomous run); implementation dispatching — Helder device smoke 16C.1 before ✅. Pointer: `DevCycleCraft/autocomplete-component/changes/2026-07-19-dx-autocompleteedit-replacement/`. |
