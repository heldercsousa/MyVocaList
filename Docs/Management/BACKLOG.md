...
| 2026-07-11 | ↳↳ Build new MD3-compliant autocomplete component (②) | 🟡 In Progress | Goal: `AutocompleteMobileField` full-screen phone variant (desktop dropdown unchanged). Gate: BUG-043 fix merged — Helder on-device re-verification pending before ✅. Pointer: `DevCycleCraft/autocomplete-component/`. |
| 2026-07-12 | ↳↳ BUG-043: release build returns zero autocomplete suggestions (Critical) | 🟡 In Progress | Goal: suggestions must appear on-device. Gate: Helder E2E 2026-07-15 confirms suggestions now appear (emulator + S23), but the fix introduced regressions BUG-044/BUG-045 — close only after those are triaged; then delete probe branch/worktrees. Pointer: `DevCycleCraft/autocomplete-component/task-log.md`. |
| 2026-07-15 | ↳↳ BUG-044: duplicate PersonFormPage after Save via autocomplete — second save creates duplicate entity (Critical) | 💡 Pending | Goal: Save must return to the singers list; instead a second form instance (pre-filled with the typed autocomplete text) is revealed and saving it inserts a duplicate row. Gate: regression of the BUG-043 fix — reproduced on emulator and S23 2026-07-15. Pointer: `DevCycleCraft/autocomplete-component/task-log.md`. |
| 2026-07-15 | ↳↳ BUG-045: name entry cursor stuck at leading position after autocomplete usage (Major) | 💡 Pending | Goal: cursor must be placeable at the end of the typed name; after using the autocomplete the cursor snaps to the start, blocking edits/save (seen on S23 2026-07-15). Gate: regression of the BUG-043 fix. Pointer: `DevCycleCraft/autocomplete-component/task-log.md`. |
