...
| 2026-07-15 | ↳↳ BUG-047: tapping an autocomplete result loads PersonFormPage with malformed name entry; with two identically named entities both render (Major) | 💡 Pending | Goal: tapped entity must load cleanly — name entry currently renders with a weird height plus artist name/birthday/email content, and duplicates render twice (evidence JPGs in `bugs/bug-043/`, incl. `ListItem component not used.png`). Registered by Helder 2026-07-15 (on-device). Pointer: `DevCycleCraft/autocomplete-component/bugs/bug-043/`. |
