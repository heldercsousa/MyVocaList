...
| 2026-06 | ↳ Crash Reporting — MaxBreadcrumbs cap | 💡 Pending | AC-CRASH-03 specifies max 50 breadcrumbs; SDK default is 100. Add `o.MaxBreadcrumbs = 50` to `LoggingConfiguration.Build` Sentry sink options. Low severity. |
| 2026-06 | ↳ Crash Reporting — integration smoke test | 💡 Pending | Trigger a test exception in a Sentry sandbox project and verify event shape in dashboard (stack trace, device tags, breadcrumbs, session ID). Do before first store release. |
| 2026-06 | ↳ Crash Reporting — multi-env Sentry DSN | 💡 Pending | Current plan embeds one DSN at build time (gitignored appsettings.json). If staging vs production ever need different DSNs, introduce appsettings.Release.json (gitignored) + build-config selection. Track before first store release. |
| 2026-06 | ↳ Crash Reporting — fill Sentry DSN | 💡 Pending | Create project at sentry.io, copy DSN into `MyVocaList/appsettings.json` (gitignored). Required before crash reports flow. |
| 2026-06 | **Crash & Error Reporting** | ✅ Done | Sentry MAUI SDK + Serilog sink; fixes orphaned Serilog config + uninitialized GlobalExceptionHandler. Spec: `Docs/Management/BusinessFeatures/crash-reporting/` · Plan: `Docs/Management/BusinessFeatures/crash-reporting/plan.md` |
