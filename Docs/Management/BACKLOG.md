...
| 2026-06 | **Enforce Git Worktrees for Parallel Subagents** | 💡 Pending | Parallel subagents collide on `git add`/`git commit` when running in the same working tree — one session unstages the other's files. Fix: update `orchestrator.md` + `workflow.md` to mandate that every parallel-wave subagent operates in its own git worktree (via `superpowers:using-git-worktrees`). Ensure the worktree pattern is the default, not opt-in, whenever ≥2 subagents are dispatched concurrently. |
| 2026-06 | **Haiku Model Assignment for Low-Risk Subagent Tasks** | 💡 Pending | As parallel task count grows, token cost scales linearly. Evaluate which subagent task types (docs-only, boilerplate generation, test scaffolding, migration-only) can be reliably handled by `claude-haiku-4-5` vs Sonnet. Define a classification rule and update orchestrator briefing templates to set `model: haiku` for eligible task types. Gate: must confirm Haiku produces correct output for a representative sample task before encoding the rule. |
