...
| 2026-06 | ↳ YouTubeSearchPage |  🔴 Blocked | Blocked on SongPickerPage + **monetisation decision**. Single developer key is ToS-compliant but free quota = 100 searches/day total across all users. Quota increases require paid tier (expensive — hundreds to thousands USD/month at scale). Central-key model only viable under a paid subscription. Pre-monetisation, `YouTube Share Intent` (oEmbed, zero cost) is the better path. See `youtube-share/findings.md`. Brief: `search-picker/task-3c-youtube-search-page.md` |
