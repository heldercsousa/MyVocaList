...
- [ ] **T1 — Create `SearchBar` component** (`UI/Components/AppBars/SearchBar.xaml(.cs)`, subclass `AppBarBase`): 56dp pill, `SurfaceContainerLow`, leading `search_outlined` (non-interactive), transplanted `TextEdit` config, `SearchText`/`Placeholder` BPs, `IsElevated` → `SurfaceContainer`, no auto-focus, no BackCommand.
...
- [ ] **T2 — Integrate `SearchBar` into `CrudListView`**: new top row; `SearchText` (TwoWay) + elevation BPs; re-point `SearchPlaceholder` to the live placeholder (update its doc-comment); verify FilterContent row stacks below.
