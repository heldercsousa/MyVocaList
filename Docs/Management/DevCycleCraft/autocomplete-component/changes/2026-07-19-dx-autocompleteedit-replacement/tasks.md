...
- [ ] **T1 — Pin DX 25.2.4 `AutoCompleteEdit` API surface** `[SPIKE-lite]`
  Produces: `findings.md` section — exact member names for async suggestions (provider/event), suggestion delay/debounce, client-filter disable, error display, item template, text-retention behavior. Sources: DevExpress MCP (verify index health first — MCP Availability Gate), Context7 (version-pinned). Escalate to Helder if unconfirmable. Consumes: —. Risk: doc gap. Files owned: `findings.md`.
- [ ] **T2 — MaterialStyles.xaml: AutoCompleteEdit form style** (REQ-DXAC-12)
  Consumes: T1. Files owned: `MyVocaList/Resources/Styles/MaterialStyles.xaml`. Demo: style resource compiles; matches Outlined TextEdit convention.
- [ ] **T3 — SongFormPage Artist field swap** (REQ-DXAC-01/03/04/05/06/07)
  Consumes: T1, T2. Files owned: `MyVocaList/UI/Pages/Songs/SongFormPage.xaml` (+ `SongFormViewModel.cs` only if debounce glue is needed — with unit tests). Demo: typing shows artist suggestions; selection locks field; blur validation intact; typed text never cleared.
