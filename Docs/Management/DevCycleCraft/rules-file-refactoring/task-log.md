...
### OPEN ISSUE 2026-07-05 (unresolved at session end — plugin-skill suppression does NOT work)
Verified across **3 `/context` loads incl. a genuinely fresh session:** the `skillOverrides` suppressions for the **superpowers PLUGIN** skills never take effect. `test-driven-development` (~40), `requesting-code-review` (~50), `receiving-code-review` (~90), `subagent-driven-development` (~40) remain fully listed.
- **Proven NOT a reload/precedence problem:** in the same fresh listing, `myvocalist-coding` and `review` show **<20 tok** (their `name-only` local overrides ARE applied), and `maui-unit-testing` toggled fine. So `settings.local.json` skillOverrides works — **but only for Project/User skills, not Plugin skills.**
- **Conclusion:** `skillOverrides` keyed by **bare skill name does not match plugin-provided skills.** They almost certainly need a **namespaced key** (candidates, UNVERIFIED: `superpowers:test-driven-development`, `test-driven-development@superpowers`, or `…@claude-plugins-official`). I did NOT guess a third time.
- **NEXT STEP (resumable):** confirm the exact plugin-skill `skillOverrides` key syntax via official Claude Code docs / `claude-code-guide` agent (WebSearch/WebFetch were just loaded when the session was closed), apply ONE clean edit to `settings.local.json`, then ONE restart to verify. **OR** decide to accept the full superpowers plugin (all skills ~600 tok total — trivial vs the ~8k saved) and instead extend the CLAUDE.md § Methodology Authority Hierarchy override line to also name `subagent-driven-development` + the code-review skills.
- **Not blocking / low-risk:** token cost of the unwanted skills is ~250 tok; the behavioral conflict (model auto-following the TDD Iron Law / brainstorming design-gate) is ALREADY neutralized by the committed Task 12 CLAUDE.md authority line (covers brainstorming + test-driven-development explicitly). The current bare-name suppression entries in `settings.json`/`settings.local.json` are harmless no-ops for the plugin skills — left in place (documented here) rather than churning config further at session end.
