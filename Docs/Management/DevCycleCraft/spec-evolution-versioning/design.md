...
> - **Phase 5 gate is redefined** to three checks: **(G1)** `regen --check` run twice yields zero diff (idempotency, REQ-SEV-13). **Precondition (T9e-F1):** `cmd_regen` does `if errors: return 2` *before* `render_archive` runs, so G1 only exercises the archive renderer once validation passes — the phase-4 wave must clear every validation error (banned content, missing `closed`, etc.) first, or invoke `_render_all`/`render_archive` directly, else G1 is a green-looking gate that never reached the renderer. **(G2)** every `BUG-NNN` enumerated in the frozen snapshot `migration/BACKLOG-pre-migration.md` is still grep-reachable in some archive file (REQ-SEV-18/20) — mechanical: grep each ID, fail on any miss. **(G3)** every archive diff line vs the pre-rewrite files must **pattern-match one of exactly three transforms** — `↳`-prefix removal, `Goal:`/`Gate:` Notes-canonicalization, or `(under: <parent>)` suffix; **any diff line matching none of the three is a defect** (this is the mechanical form — not "was it written down"). This **named archive-migration diff class** is kept *separate* from REQ-SEV-25's four active-BACKLOG classes (those remain unwidened — REQ-SEV-25 governs the live BACKLOG.md gate, not the archives).
