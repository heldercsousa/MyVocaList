...
- **REQ-SEV-08** `status` is one of the valid BACKLOG statuses; `severity` one of `Critical|Major|Minor`; `target` an ANSI date or `YYYY-MM` or `—`. Any other value is a validation error (REQ-SEV-19). *(Spec updated 2026-07-23, F-3: the set is extended beyond the original eight with terminal `Superseded` and `Duplicate` states so archived rows carrying `🔵 Superseded` / `🔵 Duplicate` validate and reproduce faithfully. `model.STATUSES`/`render.py` change in the archive-rewrite wave, with tests. **Two disambiguation rules the `🔵` glyph forces:** (1) `🔵` is overloaded — `🔵 Deferred` is an **active** live status (REQ-SEV-16) while `🔵 Superseded`/`🔵 Duplicate` are **terminal**; therefore terminal detection MUST key on the **full status string** via an explicit `TERMINAL` membership set, never on the `🔵` emoji prefix, or a Superseded row leaks into the live BACKLOG and a Deferred row is mis-archived. (2) The stored `status:` value is the **bare** enum (`🔵 Superseded`), NOT the committed cell's `🔵 Superseded (closed 2026-07-10)`; `render.py` reconstructs the `(closed <month>)` suffix from the `closed:` frontmatter key — the single source of truth for the closed date — so the two never drift.)*
...
- **REQ-SEV-21** Validation failures are reported with folder path + reason and **abort** the write — BACKLOG.md is never left partially generated. The error set is: bad `status`/`severity`/`target` value · missing required key · missing `closed` on a terminal item · Notes over the REQ-SEV-09 bound · banned content · duplicate `id` · `parent` naming no existing item · **a folder whose `severity` is `Minor`** (REQ-SEV-03) · nested/unsupported YAML (NFR-1). *(Spec updated 2026-07-23, Helder decision "allow + warn": `parent` disagreeing with the folder's path parent is a **non-fatal warning**, NOT an abort-error — cross-filing is permitted (e.g. BUG-022 lives under `persons/bugs/` but declares `parent: ui-form-validation-guide`). Aligns to design §2, which always specified WARN; this corrects the earlier abort-error wording that contradicted the design.)*
...
- **REQ-SEV-28a** *(added 2026-07-23, F-1)* Archived **non-bug sub-rows** whose pointer is a shared feature `task-log.md` (e.g. Search-Picker page sub-rows, form-validation 01–06, crud-dedup Steps 1–7e, rules-refactoring sub-tasks) each become a `changes/<slug>/README.md` under their parent feature; the `pointer:` stays on the shared `task-log.md` (REQ-SEV-27, nothing deleted). Their `id`/slug/`title`/`order` are agent-authored from the row label and flagged for the gate audit — no design model existed for these before this decision. **T12a must pin these to an exact enumerated list** (the 43 log-pointer rows partition as 21 `cross-cutting-log.md` + N `task-log.md` sub-rows; the `~22` is provisional until the inventory pins N so the partition sums to 43 with no unassigned row). **Also verify at T12a:** no *bug* sub-row points at a shared `task-log.md` outside REQ-SEV-26's enumerated list (BUG-027/029/030/031/032, 050/051/052) — such a row would fall between REQ-SEV-26 and -28a; a one-line grep confirms none exists.
...
- **REQ-SEV-30** These files are amended **together**, in one `amend:` commit with a changelog entry. Since the routing-table refactor the authoritative prose lives in the library section files, so amending the routing tables alone would leave the library contradicting them on day one — a live SDD-Invariant violation. The complete list:
  ...
  | `.claude/agents/orchestrator.md` | *(added 2026-07-27 by the mandatory sweep)* read-scope allow-list no longer sends the orchestrator to `BACKLOG.md` at session start — it names the query |
  | `.claude/exception-registry.md` | *(added 2026-07-27)* the 2026-07-22 "docs land on develop" row is **deleted**, not renewed — T13d is the protocol that replaces it |
  | `.claude/library/workflow-rule-2.md` | *(added 2026-07-27)* new § Generated artifacts — write-ownership protocol (T13d) |
  | `.claude/rules/constraints-registry.md` + `.claude/library/constraints-reference.md` | *(added 2026-07-27, FUP-3)* the stale `.sln` Solution-Folder GUID counter `0041` → `00D3`, and a note that `register` now allocates GUIDs |
  | `Docs/Changelog/changelog.md` | *(added 2026-07-27)* the `amend:` entry required by `CLAUDE.md § Amending These Rules` (T13c) |
  ...
  > **Spec updated [2026-07-27]:** the mandatory sweep was run at T13 preparation time and returned **two hits outside the original list** — `.claude/agents/orchestrator.md` (its read-scope allow-list still named `BACKLOG.md` as a session-start read, which Rule 7's replacement contradicts) and `.claude/exception-registry.md`. Per this requirement's own instruction both are now table rows rather than a note in the bundle. Four further files are added in the same pass because tasks written after this requirement (T13d, T13c, FUP-3) own edits in them; leaving them out would reproduce exactly the contradiction REQ-SEV-30 exists to prevent — routing tables amended while the files they point at still describe the old rule. Sweep evidence and the OLD→NEW text for every row: `T13-proposed-diffs.md`.
...
- **REQ-SEV-32** An archived row whose `pointer` targets a file **outside** the `Docs/Management` item tree (e.g. `.claude/rules/*.md`, `Docs/Changelog/changelog.md`) still gets a **minimal `cross-cutting/<slug>/README.md` folder** during migration, with `pointer:` set to the external file unchanged — it does NOT go folder-less. *(Spec updated 2026-07-24, T12a Wave C: written ahead of the rows that need it — the 3 affected rows ship folder-less in later waves per `T12a-remaining-waves-plan.md` Blocker #1.)* > **Spec updated 2026-07-25:** superseded above — the canonical `regen` rewrite reconstructs an archive file entirely from folder frontmatter (REQ-SEV-27's generated-file model), so a folder-less row is silently dropped on the first rewrite (hazard H1). Per Helder's 2026-07-25 ruling, REQ-SEV-27 ("nothing deleted") wins: these rows get a minimal `cross-cutting/<slug>/README.md` folder like any other archived item, with `pointer:` carrying the external file path unchanged. The "no folder" reading above was correct for a hand-spliced archive but not for a fully-generated one; it is superseded, not retracted from history.
