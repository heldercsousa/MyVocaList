...
| INLINE-AC | Song artist field — correctness fixes (BUG-050/051/052 + retain-text) + inline create-new-artist | SPEC APPROVED by Helder (2026-07-21, spec-reviewer PASS); planning next in a fresh session (see feature `handoff.md`) | — (docs on develop) | — | spec approved | (this commit) | Next session (PLANNING): writing-plans → plan.md → plan-reviewer → Helder plan approval. Then a separate session for implementation in a worktree (T1 BUG-050 first, regression-test-first). Read feature `handoff.md` first. |
