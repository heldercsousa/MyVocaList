...
| INLINE-AC | Song artist field — correctness fixes (BUG-050/051/052 + retain-text) + inline create-new-artist | consolidated spec (post DX-AC T7) + spec-reviewer PASS (2026-07-21); awaiting Helder spec approval, then plan | — (docs on develop) | — | to review | 5f5d7b9 | Helder: approve spec in `BusinessFeatures/artists-songs/changes/2026-07-21-inline-artist-create/`; on approval → writing-plans → plan-reviewer → implement in worktree (T1 BUG-050 first, regression-test-first) |
