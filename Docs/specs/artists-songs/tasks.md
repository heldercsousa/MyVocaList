...
> **Status:** Phase 8 — final smoke test pending
...
- [x] **5.1** Register `ArtistService`, `SongService`, `MusicMetadataService` as `AddScoped` in `MauiProgram.cs`
- [x] **5.2** Register `MusicBrainzProvider`, `DeezerProvider` as `AddScoped<IMusicMetadataProvider>` (order: MusicBrainz first)
- [x] **5.3** Register `HttpClient` for each provider via `AddHttpClient<T>` with base address and `User-Agent`
- [x] **5.4** Register `ArtistRepository`, `SongRepository` as `AddScoped`
- [x] **5.5** Register all pages and ViewModels as `AddTransient`
- [x] **5.6** Add routes `ArtistForm`, `Songs`, `SongForm` to `Routes.cs`
- [x] **5.7** Register routes in `AppShell.xaml.cs`
- [x] **5.8** Build — 0 errors
...
- [x] **6.2** Complete `ArtistsPage.xaml.cs` — all code-behind event handlers per design; build after
- [x] **6.3** Implement `ArtistsViewModel` — all properties, commands, selection logic, confirm sheet state; build after
- [x] **6.4** Implement `ArtistFormPage.xaml` — Name field + character counter + suggestion list + API search strip + API results + overwrite warning `dx:BottomSheet` + action buttons; build after
- [x] **6.5** Implement `ArtistFormPage.xaml.cs` — `OnAppearing` focus, `OnBackButtonPressed`, overwrite sheet state sync; build after
...
- [x] **6.7** Run `/project:review` — 0 issues before proceeding

---

## Phase 7 — Songs UI

- [x] **7.1** Implement `SongsPage.xaml` — mirrors Artists page; artist name in `SmallAppBar` title; song rows with `FeaturedArtists` supporting text; build after
- [x] **7.2** Implement `SongsPage.xaml.cs`; build after
- [x] **7.3** Implement `SongsViewModel` — receives `ArtistId` + `ArtistName` via query; same structure as `ArtistsViewModel` except `AppBarTitle` always = artist name and `AppBarSubtitle` shows "N selected" (not the title); build after
- [x] **7.4** Implement `SongFormPage.xaml` — artist read-only label + Title field + FeaturedArtists field + API search strip + action buttons; build after
- [x] **7.5** Implement `SongFormPage.xaml.cs`; build after
- [x] **7.6** Implement `SongFormViewModel` — same structure as `ArtistFormViewModel`; API search scoped to artist hint; build after
- [x] **7.7** Run `/project:review` — 0 issues before committing
