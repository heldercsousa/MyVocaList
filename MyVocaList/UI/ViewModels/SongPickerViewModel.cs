...
public sealed partial class SongPickerViewModel : ViewModelBase, IDisposable
{
    ...
    {
        _messenger.Send(new SongPickedMsg(result));
    ...
    {
        // YouTube search launch — navigates to YouTubeSearchPage with the selected song context
        await Shell.Current.GoToAsync(Routes.YouTubeSearch,
            new Dictionary<string, object> { ["Song"] = result });
    }
